...
static void delayed_input(struct k_work *work)
{
	...
	bs_bc_receive_msg(*oob_channel_id, (uint8_t *)oob_data, size);

	switch (gact) {
	case BT_MESH_PUSH:
	case BT_MESH_TWIST:
	case BT_MESH_ENTER_NUMBER:
		ASSERT_OK(bt_mesh_input_number(strtoul(oob_data, NULL, 10)));
		break;
	case BT_MESH_ENTER_STRING:
		ASSERT_OK(bt_mesh_input_string(oob_data));
		break;
	default:
...
static void oob_device(bool use_oob_pk)
{
	...
	if (use_oob_pk) {
		...
		LOG_INF("OOB Public Key: %s", log_strdup(bt_hex_real(public_key_be, 64)));
	}
